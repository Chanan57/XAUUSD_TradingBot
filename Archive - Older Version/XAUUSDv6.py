...
# --- 1. CONFIGURATION (V6.2 RAW TREND) ---
SYMBOL = "XAUUSD.a"       # CHECK BROKER SYMBOL
TIMEFRAME = mt5.TIMEFRAME_M30
RISK_PERCENT = 0.02       # 2% Risk
MAGIC_NUMBER = 888062     # V6.2 Magic Number
...
def print_log_block(timestamp, price, spread, balance, equity, prob_up, prob_down, brain_reason, trend_status, trend_reason, rsi, atr, adx, decision, risk_note=""):
    ...
    print(f"\n{'='*65}", flush=True)
    print(f" 📅 LOG: {timestamp.strftime('%H:%M:%S')} | 🦅 PHOENIX V6.2 (RAW TREND)", flush=True)
        ...
        for pos in positions:
            if pos.magic == MAGIC_NUMBER:
                ...
                req = {
                    ...
                    "comment": f"V6.2:{reason}", "type_time": mt5.ORDER_TIME_GTC, "type_filling": mt5.ORDER_FILLING_IOC
                }
                mt5.order_send(req)
        ...
        if spread_points <= allowed_spread:
            ...
            if vol > 0:
                ...
                req = {
                    ...
                    "price": p, "sl": sl, "tp": tp, "comment": "V6.2 Raw", 
...
print("\n✅ PHOENIX VERSION 6.2 (RAW TREND) INITIALIZED.", flush=True)
print("   - ADX Shield: ACTIVE (Filter < 20.0)")
print("   - Profit Lock: DISABLED (Full room to breathe)")
