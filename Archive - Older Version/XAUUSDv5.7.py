...
# --- 4. DASHBOARD DISPLAY FUNCTION ---
def print_dashboard(timestamp, price, spread, allowed_spread, balance, equity, prob_up, prob_down, trend_status, sma, ema, rsi, atr):
    ...
    print(f"| 🚀 V5.8 TITANIUM DASHBOARD       ⏰ {timestamp:<25} |", flush=True)
    print(f"+{'-'*68}+", flush=True)
    print(f"| 💰 PRICE: {price:<10.2f}    | 🛡️ SPREAD: {spread:<5.1f} (Max: {allowed_spread})       |", flush=True)
    print(f"| 💵 BALANCE: ${balance:<10.2f}  | 📉 EQUITY: ${equity:<10.2f}            |", flush=True)
    print(f"+{'-'*68}+", flush=True)
    print(f"| 🤖 TRADE STATUS: {trade_status:<46} |", flush=True)
    print(f"|    {trade_details:<62} |", flush=True)
    print(f"+{'-'*68}+", flush=True)
    print(f"| 🧠 AI BRAIN:             | 🌊 TREND:                          |", flush=True)
    print(f"|    🟢 UP:   {prob_up:<10.1%} |    {trend_status:<32} |", flush=True)
    print(f"|    🔴 DOWN: {prob_down:<10.1%} |                                    |", flush=True)
    print(f"+{'-'*68}+", flush=True)
    print(f"| 📊 TECHNICALS:                                             |", flush=True)
    print(f"|    SMA(200): {sma:<10.2f}  | EMA(50): {ema:<10.2f}                |", flush=True)
    print(f"|    RSI:      {rsi:<10.1f}  | ATR:     {atr:<10.2f}                |", flush=True)
    print(f"+{'-'*68}+", flush=True)
...
def perform_analysis(model, predictors):
    ...
    # CALL NEW DASHBOARD PRINT
    print_dashboard(datetime.now().strftime('%Y-%m-%d %H:%M:%S'), last_row['close'], spread_points, allowed_spread, 
                    acc.balance, acc.equity, prob_up, prob_down, trend_status, 
                    last_row['SMA_200'], last_row['EMA_50'], last_row['RSI'], last_row['ATR'])
