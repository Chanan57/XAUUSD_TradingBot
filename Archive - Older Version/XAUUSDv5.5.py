            ...
            if acc and sym_info:
                ...
                if vol > 0:
                    ...
                    req = {"action": mt5.TRADE_ACTION_DEAL, "symbol": SYMBOL, "volume": vol, "magic": MAGIC_NUMBER,
                           ...
                           "price": p, "sl": sl, "tp": tp, "comment": "AI V5.4 Recov", 
                           "type_time": mt5.ORDER_TIME_GTC, "type_filling": mt5.ORDER_FILLING_IOC}
                    res = mt5.order_send(req)
                    if res.retcode == mt5.TRADE_RETCODE_DONE: 
                        log_trade(signal, SYMBOL, p, sl, tp, vol, f"V5.4 Entry AI:{max(prob_up, prob_down):.2f}")
                    else:
...
while True:
    try:
        ...
        # SYDNEY SCHEDULE FIX: Retrain at 11:05 AM (After US Close chaos settles)
        if now.hour == 11 and now.minute == 5 and last_training_day != now.day:
        ...
        if rates is not None and last_candle_time != rates[0]['time']:
            ...
            # SYDNEY SCHEDULE FIX: Sleep 8 AM - 11 AM (Rollover/Spread Spike Zone)
            if 8 <= now.hour <= 10:
                print(f"💤 Rollover Hours (Sydney 8-11am). Skipping.", flush=True)
            else:
                perform_analysis(model, predictors)
