...
def log_analysis(time_stamp, price, prob_up, prob_down, rsi, sma, ema, atr, trend_status):
    filename = "Gold_Strategy_Analysis_V5.csv"
    file_exists = os.path.isfile(filename)
    with open(filename, mode='a', newline='', encoding='utf-8') as file:
        writer = csv.writer(file)
        if not file_exists:
            writer.writerow(["Date", "Time", "Price", "AI_UP", "AI_DOWN", "RSI", "SMA_200", "EMA_50", "ATR", "Trend_Status"])
        ...
        writer.writerow([d_str, t_str, price, f"{prob_up:.2f}", f"{prob_down:.2f}", 
                         f"{rsi:.1f}", f"{sma:.2f}", f"{ema:.2f}", f"{atr:.2f}", trend_status])
...
def perform_analysis(model, predictors):
        ...
        if (signal == "BUY" and trend_up) or (signal == "SELL" and trend_down):
            ...
            if acc and sym_info:
                ...
                req = {"action": mt5.TRADE_ACTION_DEAL, "symbol": SYMBOL, "volume": vol, "magic": MAGIC_NUMBER,
                       ...
                       "price": p, "sl": sl, "tp": tp, "comment": "AI V5 Live", 
                       "type_time": mt5.ORDER_TIME_GTC, "type_filling": mt5.ORDER_FILLING_IOC}
                res = mt5.order_send(req)
                if res.retcode == mt5.TRADE_RETCODE_DONE: 
                    log_trade(signal, SYMBOL, p, sl, tp, vol, f"V5 Entry AI:{max(prob_up, prob_down):.2f}")
                else:
    ...
    log_analysis(datetime.now(), last_row['close'], prob_up, prob_down, last_row['RSI'], last_row['SMA_200'], last_row['EMA_50'], last_row['ATR'], trend_status)
    ...
    print(f"ww Trend Check: {trend_status}", flush=True)
