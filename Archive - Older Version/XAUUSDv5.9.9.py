...
# --- 1. CONFIGURATION (V5.9.7 IRON CLAD) ---
SYMBOL = "XAUUSD.a"       # CHECK BROKER SYMBOL
TIMEFRAME = mt5.TIMEFRAME_M30
RISK_PERCENT = 0.02       # 2% Risk
MAGIC_NUMBER = 888012     # Updated Magic Number
MAX_SPREAD_NORMAL = 35    
MAX_SPREAD_HIGH_CONF = 60 

# --- VOLATILITY SHIELD (IRON CLAD) ---
# Hard Cap: If 0.01 lots would lose more than $40 AUD, BLOCK THE TRADE.
MAX_SURVIVABLE_LOSS_AUD = 40.00 
...
def print_log_block(timestamp, price, spread, balance, equity, prob_up, prob_down, brain_reason, trend_status, trend_reason, rsi, atr, sma, ema, decision, risk_note=""):
    """Prints a detailed block including RECOVERY PATH"""
    ...
    print(f"\n{'='*65}", flush=True)
    print(f" 📅 LOG ENTRY: {timestamp} | 🦅 PHOENIX V5.9.7 (IRON CLAD)", flush=True)
    print(f"{'-'*65}", flush=True)
    print(f" 💰 Price:   {price:<10.2f}    🛡️ Spread: {spread:<5.1f}", flush=True)
    # UPDATED DASHBOARD LINE:
...
# --- 5. RISK ENGINE (IRON CLAD AUD FIX) ---
def get_exchange_rate(base_currency, target_currency):
    """Gets the conversion rate (e.g., AUDUSD)"""
...
def get_lot_size(balance, risk_pct, sl_dist, sym_info):
    ...
    sym_currency = sym_info.currency_profit
    exchange_rate = get_exchange_rate(acc_currency, sym_currency)
    
    # 2. IRON CLAD CHECK: "If I trade 0.01 lots, how much do I lose in AUD?"
    min_lot_risk_usd = 0.01 * contract_size * sl_dist
    min_lot_risk_aud = min_lot_risk_usd / exchange_rate 

    # 3. THE VOLATILITY SHIELD 
    if min_lot_risk_aud > MAX_SURVIVABLE_LOSS_AUD:
        return 0.0, 0.0, f"⛔ BLOCKED: Min Risk ${min_lot_risk_aud:.2f} > Limit ${MAX_SURVIVABLE_LOSS_AUD:.2f}"

    # 4. Standard Calc (If Shield Pass)
    ...
    # 7. Recalculate Actual Risk for Display
        ...
        for pos in positions:
            if pos.magic == MAGIC_NUMBER:
                ...
                req = {
                    ...
                    "comment": f"V5.9.7:{reason}", "type_time": mt5.ORDER_TIME_GTC, "type_filling": mt5.ORDER_FILLING_IOC
                }
                mt5.order_send(req)
...
def perform_analysis(model, predictors):
    ...
    if current_pos is None:
        if prob_up > CONFIDENCE_ENTRY and trend_up: signal = "BUY"; decision_text = "ENTRY SIGNAL (BUY)"
        elif prob_down > CONFIDENCE_ENTRY and trend_down: signal = "SELL"; decision_text = "ENTRY SIGNAL (SELL)"
        else: decision_text = f"HOLD (Conf: {max(prob_up, prob_down):.2f})"
    elif current_pos == "BUY" and prob_down > CONFIDENCE_REVERSAL: 
        signal = "SELL"; decision_text = "REVERSAL (Flip to SELL)"
    elif current_pos == "SELL" and prob_up > CONFIDENCE_REVERSAL: 
        signal = "BUY"; decision_text = "REVERSAL (Flip to BUY)"
    else:
        ...
        if spread_points <= allowed_spread:
            ...
            if vol > 0:
                ...
                req = {
                    ...
                    "price": p, "sl": sl, "tp": tp, "comment": "V5.9.7 IronClad", 
    ...
    print_log_block(
        ...
        last_row['RSI'], last_row['ATR'], sma, ema, decision_text, risk_warning
    )
...
print("\n✅ V5.9.7 PHOENIX (IRON CLAD) INITIALIZED.", flush=True)
...
while True:
    try:
        ...
        if rates is not None and rates[0]['time'] != last_candle_time:
            last_candle_time = rates[0]['time']; perform_analysis(model, predictors)
        time.sleep(5) 
